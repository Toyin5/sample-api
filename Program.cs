...
app.MapPost("/upload", async ([FromHeader(Name ="Key")] string apiKey, [FromBody]FileDto file, UserRepository userRepository) =>
{
    var result = await userRepository.UploadAsync(apiKey,file);
    return result;
})
.WithName("Upload")
.WithOpenApi();
